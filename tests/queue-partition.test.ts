...
function queueAt(world: TempWorld, queueDir: string): EventQueue {
...
function queueFor(world: TempWorld, token: string | undefined): EventQueue {
  return queueAt(world, queuePartition(path.join(world.home, 'queue'), token));
}

/** A queue the way a pre-partition edge wrote it: flat in the root. */
function legacyQueue(world: TempWorld): EventQueue {
  return queueAt(world, path.join(world.home, 'queue'));
}
...
describe('queue partitions', () => {
  ...
  it('adopts an unpartitioned backlog when the machine has one identity', async () => {
    await legacyQueue(world).enqueue([makeEvent('evt_legacy')], 'https://backend.example.com/v1/events');
  ...
  it('refuses to guess an owner when the machine serves several, and loses nothing', async () => {
    await legacyQueue(world).enqueue([makeEvent('evt_ambiguous')], 'https://backend.example.com/v1/events');
  ...
  it('does nothing at all until an identity exists to adopt into', async () => {
    await legacyQueue(world).enqueue([makeEvent('evt_legacy')], '*');

    expect(await settleLegacyQueue(root, undefined, false)).toBe(0);
    expect(await queueEntryFiles(root)).toHaveLength(1);
  });
});
